    ...
    void Update() {
        ...
        if (!smashing && !laggin) {
            if (Mathf.Abs(Input.GetAxis("Horizontal" + playerControl)) > 0.1f) {
                xSpeed += speed * Input.GetAxis("Horizontal" + playerControl) / 32;
                GetComponent<SpriteRenderer>().flipX = Input.GetAxisRaw("Horizontal" + playerControl) < 0;
                anim.SetFloat("velocity", 1);
                slopeCheck();
            } else {
                xSpeed = Mathf.Lerp(xSpeed, 0, Time.deltaTime * 4);
            }
            ...
            if (Input.GetButtonDown("Smash" + playerControl) && canSmash && !smashing && !touchingGround) {
